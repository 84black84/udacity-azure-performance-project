...
handler = AzureLogHandler(connection_string='InstrumentationKey=932d0113-4d57-42f7-b901-3d225029b033')
handler.setFormatter(logging.Formatter('%(traceId)s %(spanId)s %(message)s'))
logger.addHandler(handler)
# Logging custom Events 
logger.addHandler(AzureEventHandler(connection_string='InstrumentationKey=932d0113-4d57-42f7-b901-3d225029b033'))
...
exporter = metrics_exporter.new_metrics_exporter(
    ...
    connection_string='InstrumentationKey=932d0113-4d57-42f7-b901-3d225029b033')
view_manager.register_exporter(exporter)

# Tracing
# Setup tracer 
tracer = Tracer(
 exporter=AzureExporter(connection_string='InstrumentationKey=932d0113-4d57-42f7-b901-3d225029b033'),
...
# rredisConnection = redis.Redis()

# Redis configurations
redis_server = os.environ['REDIS']

# Redis Connection to another containertry:
try:
    if "REDIS_PWD" in os.environ:
        redisConnection = redis.StrictRedis(host=redis_server,
                        port=6379,
                        password=os.environ['REDIS_PWD'])
    else:
        redisConnection = redis.Redis(redis_server)
